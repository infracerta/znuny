        ...
        function InstallZnuny
        {
                ...
                ln -s ${ZNUNY_INSTALL_DIR}/scripts/apache2-httpd.include.conf /etc/apache2/conf-enabled/ 1> /dev/null
                if [ $? = 0 ]; then
                        echo "OK"
                else
                ...
                mv ${ZNUNY_INSTALL_DIR}/Kernel/Config.pm.dist ${ZNUNY_INSTALL_DIR}/Kernel/Config.pm 1> /dev/null
                if [ $? = 0 ]; then
                        echo "OK"
                else
                ...
                useradd -d ${ZNUNY_INSTALL_DIR}/ -s /bin/bash -c 'Znuny user' znuny 1> /dev/null
                usermod -G www-data znuny 1> /dev/null
                ${ZNUNY_INSTALL_DIR}/bin/otrs.SetPermissions.pl --znuny-user znuny --web-group www-data ${ZNUNY_INSTALL_DIR} 1> /dev/null
                if [ $? = 0 ]; then
                        echo "OK"
                else
                ...
                cd ${ZNUNY_INSTALL_DIR}/var/cron/ && for foo in *.dist; do cp $foo `basename $foo .dist`; done
                chown znuny:www-data ${ZNUNY_INSTALL_DIR}/var/cron/*_daemon
                if [ $? = 0 ]; then
                        echo "OK"
                else
                        echo "Ocorreu durante essa etapa, verifique o log acima";exit
                ...
                echo ""
                echo "OTRS/Znuny instalado com sucesso! Acesse o endereço http://$(ip route get 8.8.8.8 | awk -F"src " 'NR==1{split($2,a," ");print a[1]}')/znuny/installer.pl para finalizar a configuração do sistema."
